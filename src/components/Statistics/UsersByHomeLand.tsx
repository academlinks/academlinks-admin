...
import { selectStatisticByHomeland } from "../../store/selectors/statisticSelectots";
import { useStatistic } from "../../hooks";

ChartJS.register(ArcElement, Tooltip, Legend);

const UsersByHomeLand: React.FC = () => {
  const { labels, range, total, countriesTotal } = useAppSelector(
    selectStatisticByHomeland
  );
  
  const [chartData, setChartData] = useState<any>();

  const { setHomelandRange } = useStatistic();

  useEffect(() => {
    if (typeof range[0] !== "number") return;

    setChartData(getChartConfig({ key: "homelandRange", data: range, labels }));
  }, [range]);
...
export default UsersByHomeLand;
